...
BIAS_TERMS_DICT = {
    ...
    # #dora
    # 'key': ['attention.self.key.A', 'attention.self.key.B','attention.self.key.lora_m'],
    # 'query':['attention.self.query.A', 'attention.self.query.B','attention.self.query.lora_m'],
    # 'value':['attention.self.value.A', 'attention.self.value.B','attention.self.value.lora_m'],
    # lora
    'key': ['attention.self.key.A', 'attention.self.key.B'],
    'query':['attention.self.query.A', 'attention.self.query.B'],
    'value':['attention.self.value.A', 'attention.self.value.B'],
    #vera
    # 'key': ['attention.self.key.real_b', 'attention.self.key.real_d'],
    # 'query':['attention.self.query.real_b', 'attention.self.query.real_d'],
    # 'value':['attention.self.value.real_b', 'attention.self.value.real_d'],
    # #vera1d
    # 'key': ['attention.self.key.scale'],
    # 'query':['attention.self.query.scale'],
    # 'value':['attention.self.value.scale'],
...
class LoRALinear(nn.Module):
    def __init__(self, in_features, out_features, r=4, alpha=1.0, bias=True):
        ...
        self.alpha = alpha
        if r > 0:
            self.A = nn.Parameter(torch.zeros(r,32))
            self.B = nn.Parameter(torch.zeros(24, r))
    ...
    def forward(self, x):
        if self.r > 0:
            lora_out = (self.alpha / self.r) * (self.A.T @ self.B.T).flatten()
...
class VeraLinear(nn.Module):
    def __init__(self, in_features, out_features, r=4, alpha=1.0, bias=True):
        ...
        if r > 0:
            self.A = nn.Parameter(torch.zeros(r,32))
            self.B = nn.Parameter(torch.zeros(24, r))
            self.real_b = nn.Parameter(torch.zeros(1,24))
            self.real_d = nn.Parameter(torch.zeros(1,r))

            nn.init.kaiming_uniform_(self.A, a=math.sqrt(5))
            nn.init.kaiming_uniform_(self.B, a=math.sqrt(5))
            nn.init.kaiming_uniform_(self.real_d, a=math.sqrt(5))
            nn.init.kaiming_uniform_(self.real_b, a=math.sqrt(5))
    ...
    def forward(self, x):
        if self.r > 0:
            lora_out = (self.alpha / self.r) * ((self.real_d.T*self.A).T @ (self.real_b.T*self.B).T).flatten()
            return self.linear(x) + lora_out
        else:
            return self.linear(x)

class VeraLinear1D(nn.Module):
    def __init__(self, in_features, out_features, r=4, alpha=1.0, bias=True):
        super().__init__()
        self.linear = nn.Linear(in_features, out_features, bias=bias)
        self.r = r
    ...
    def forward(self, x):
        if self.r > 0:
            lora_out = (self.alpha / self.r) * self.scale
            return self.linear(x) + lora_out
        else:
            return self.linear(x)

class DoraLinear(nn.Module):
    def __init__(self, in_features, out_features, use_date, r=4, alpha=1.0, bias=True):
        super().__init__()
        self.linear = nn.Linear(in_features, out_features, bias=bias)
        ...
        self.V = self.use_date.detach()
        m_init = (self.use_date.norm(dim=0, keepdim=True)).detach() 
        
        if r > 0:
            self.A = nn.Parameter(torch.zeros(r,32))
            self.B = nn.Parameter(torch.zeros(24, r))
            self.lora_m = nn.Parameter(m_init)
    ...
    def forward(self, x):
        if self.r > 0:
            delta_V = self.B @ self.A
            V_new = delta_V.flatten() + self.V.cuda()
            V_new = V_new / V_new.norm(dim=0, keepdim=True)
            W_prime = V_new * self.lora_m*(self.alpha / self.r)       
            return self.linear(x) + W_prime
        else:
            return self.linear(x)
...
class GLUEvaluator:
    ...
    def replace_with_lora(self,module, r=8, alpha=1):
        for name, child in module.named_children():
            if isinstance(child, torch.nn.Linear) and (self.bias_terms[0] in name):
                print('replacing ', name, ' with LoRA layer')
                lora_linear = LoRALinear(child.in_features, child.out_features, r=r, alpha=alpha, bias=child.bias is not None)
                lora_linear.linear.weight.data = child.weight.data.clone()
                if child.bias is not None:
                    lora_linear.linear.bias.data = child.bias.data.clone()
                setattr(module, name, lora_linear)
            else:
                #print('traversing ', name, '...')
                self.replace_with_lora(child, r, alpha)
    ...
    def training_preparation(self, learning_rate, optimizer, encoder_trainable, trainable_components=None,
        ...
        self.replace_with_lora(self.model, r=8, alpha=1)
    ...
    def convert_to_actual_components(components):
        return [item for component in components 
                for item in BIAS_TERMS_DICT[component]]
