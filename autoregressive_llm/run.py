...
class Framework:
    ...
    def load_model(self):
        ...
        if self.args.bias_term=='v':
            target_keywords = [
                "v_proj.bias",
            ]
        elif self.args.bias_term =='q':
            target_keywords = [
                "q_proj.bias",
            ]    
        elif self.args.bias_term =='k':
            target_keywords = [
                "k_proj.bias",
            ]    
        elif self.args.bias_term == None:
            print('not do special bias ft')               


        if self.args.bias_term != None:
...
def main():
    ...
    if args.train_set_seed is not None or args.num_train_sets is not None:
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if not args.no_eval:
                ...
                if args.local_rank <= 0:
                    write_metrics_to_file(metrics, "./BEFT_results/" +  result_file_tag(args) + f"-trainset{train_set_id}.json" if args.result_file is None else args.result_file)
        ...
        logger.info(metrics)
        if args.local_rank <= 0:
            write_metrics_to_file(metrics, "./BEFT_results/" + result_file_tag(args) + "-onetrainpereval.json" if args.result_file is None else args.result_file)
